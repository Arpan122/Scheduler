...
const getCorsOrigins = () => {
    ...
    if (frontendEnv) {
        if (frontendEnv.startsWith("http://") || frontendEnv.startsWith("https://")) {
            ...
            if (!defaultOrigins.includes(clean))
                defaultOrigins.push(clean);
        }
        else {
            const httpUrl = `http://${frontendEnv}`;
            const httpsUrl = `https://${frontendEnv}`;
            if (!defaultOrigins.includes(httpUrl))
                defaultOrigins.push(httpUrl);
            if (!defaultOrigins.includes(httpsUrl))
                defaultOrigins.push(httpsUrl);
        }
    }
...
app.post("/api/login", async (req, res) => {
    ...
    try {
        const authHeader = Buffer.from(`${process.env.VITE_COGNITO_CLIENT_ID}:${process.env.COGNITO_CLIENT_SECRET}`).toString('base64');
        const cid = process.env.VITE_COGNITO_CLIENT_ID ? process.env.VITE_COGNITO_CLIENT_ID : "";
        ...
        const response = await axios.post(`https://${process.env.VITE_COGNITO_DOMAIN}/oauth2/token`, params.toString(), {
            headers: {
...
app.post("/api/upload", upload.single("image"), async (req, res) => {
    try {
        ...
        const text = response.text ? response.text : "No response";
